...
export default class NuevoMedicoDialog extends React.Component {
  ...
  guardarMedico = (inputs, cerrarDialog) => {
    const {
      nombre, direccion, email, comision, telefono1, telefono2,
    } = inputs

    insertarMedico(nombre, direccion, email, comision, telefono1, telefono2)
    .then((resp) => {
      this.setState({inputs: {}, errors: {}, serverError: null})
      cerrarDialog(`Nuevo medico guardado: ${nombre}`, NUEVO_MEDICO_DIALOG_CLOSED)
    }, (err) => {
  ...
  render() {
    ...
    return (
      <Dialog
        ...
        <NuevoMedicoForm inputs={this.state.inputs} errors={this.state.errors}
