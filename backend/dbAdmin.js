...
const updateVenta = (
  ...
  return builder('ventas').where({ codigo, empresa }).update({
    cliente: cliente,
    fecha: fecha,
    autorizacion: autorizacion,
    formaPago: formaPago,
...
const deleteVenta = (codigo, empresa) => {
  return knex('ventas').where({ codigo, empresa }).del();
};

const deleteProducto = rowid => {
  return knex('productos').where({ rowid }).del();
};

const deleteCliente = ruc => {
  return knex('clientes').where({ ruc }).del();
};
...
const getCliente = ruc => {
  return knex.select('*').from('clientes').where('ruc', ruc);
};
...
const buscarEnTabla = (tabla, columna, queryString, limit) => {
  ...
  const query = knex.select('*').from(tabla).limit(limitValue);
  if (queryString !== '')
